...
inline bool existe_arq (const std::string& comando) {
    ifstream f(comando.c_str());
    return f.good();
}
...
int main()
{
    //Lista de documentos dentro de /bin.
    string docs[] = {"cat", "chgrp", "chmod", "chown", "cp", "date", "dd", "df", "dmesg", "echo", "false", "hostname", "kill", "ln", "login", "ls", "mkdir", "mknod", "more", "mount", "mv", "ps", "pwd", "rm", "rmdir", "sed", "sh", "stty", "su", "sync", "true", "umount", "uname", "csh", "ed", "tar", "cpio", "gzip", "gunzip", "zcat", "netstat", "ping"};
